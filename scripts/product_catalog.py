...
SELLABLE_PRODUCTS = {
    ...
    "crm": {
        ...
        "ru": {
            ...
            "steps": ["Заявка попадает в CRM", "получает статус и ответственного", "сотрудник видит следующий шаг", "история сохраняется", "руководитель видит очередь и показатели"],
            "includes": ["клиентов и заказы", "статусы", "ответственных", "рабочую панель", "базовую аналитику"],
            "excludes": ["замену всех корпоративных систем сразу", "миграцию сложной legacy-базы без отдельного этапа", "неограниченные отчёты"],
        ...
        "en": {
            ...
            "steps": ["A lead enters the CRM", "gets a status and owner", "staff see the next action", "history is preserved", "management sees the queue and key numbers"],
